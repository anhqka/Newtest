...
import { createSocialsAsync } from "../../../../page/admin/Partials/PartialsSlice";
...
const NewSocial = () => {
    ...
    const onFinish = async (value) => {
        ...
        if (imageNew) {
            const res = await dispatch(createSocialsAsync(formData))
            if (res.meta.requestStatus == "fulfilled") {
                success("Thêm mới social thành công!")
                setOpen(false)
            } else {
                error("Thêm mới social thất bại!")
            }
        } else {
