...
import { createsocialsAsync, fetchSocialsAsync, updateBanners, updateBannersAsync, updateSocialAsync } from "page/admin/Partials/PartialsSlice";
import { warning, error, success } from "utils/messageAnt";
import { fetchBannersAsync } from "../../../../page/admin/Partials/PartialsSlice";

const UpdateSocial = ({ record }) => {
    ...
    const onFinish = async (value) => {
        ...
        if (image) {
            ...
            const res = await dispatch(updateSocialAsync(formData))
            ...
            >
                ...
                >
                    <div className='flex items-stretch space-x-3'>
                        <Form.Item label="Tên social" className="w-[50%]" required tooltip="Không được để trống">
                            <Form.Item name="name"
                                rules={[
                                    {
                                        required: true,
                                    ...
                                    {
                                        min: 6,
                                        message: "Tên social bắt buốc trên 6 ký tự!"
                                    }
                                ]}
                            >
                                <Input />
